...
// TacoSpigot end
import it.unimi.dsi.fastutil.longs.LongOpenHashSet; // SternalSpigot

public class ChunkProviderServer implements IChunkProvider {

    private static final Logger b = LogManager.getLogger();
    public LongSet unloadQueue = new LongOpenHashSet(); // CraftBukkit - LongHashSet // TacoSpigot - LongHashSet -> HashArraySet // SternalSpigot - LongOpenHashSet
    ...
    public void queueUnload(int i, int j) {
        long key = LongHash.toLong(i, j); // SternalSpigot - Only create key once
        // PaperSpigot start - Asynchronous lighting updates
        Chunk chunk = chunks.get(key); // SternalSpigot
        ...
        if (this.world.worldProvider.e()) {
            if (!this.world.c(i, j)) {
                // CraftBukkit start
                this.unloadQueue.add(key); // SternalSpigot  // TacoSpigot - directly invoke LongHash

                Chunk c = chunks.get(key); // SternalSpigot
                if (c != null) {
                    c.mustSave = true;
                }
                // CraftBukkit end
    ...
    public Chunk getChunkAt(int i, int j, Runnable runnable) {
        long key = LongHash.toLong(i, j); // SternalSpigot - Only create key once
        unloadQueue.remove(key); // SternalSpigot // TacoSpigot - directly invoke LongHash
        Chunk chunk = chunks.get(key); // SternalSpigot
    ...
    public Chunk originalGetChunkAt(int i, int j) {
        long key = LongHash.toLong(i, j); // SternalSpigot - Only create key once
        this.unloadQueue.remove(key); // SternalSpigot // TacoSpigot - directly invoke LongHash
        Chunk chunk = (Chunk) this.chunks.get(key); // SternalSpigot
            ...
            if (chunk == null) {
                if (this.chunkProvider == null) {
                    ...
                    try {
                        ...
                        crashreportsystemdetails.a("Position hash", (Object) key); // SternalSpigot // CraftBukkit - Use LongHash
    ...
    // SternalSpigot start - Optimise Chunk Getting
