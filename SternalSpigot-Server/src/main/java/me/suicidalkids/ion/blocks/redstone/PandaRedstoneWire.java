    ...
    protected void checkSurroundingWires(World worldIn, BlockPosition pos) {
        ...
        for (EnumDirection facingVertical : facingsVertical) {
            ...
            for (EnumDirection facingHorizontal : facingsHorizontal) {
                ...
                if (facingVertical == EnumDirection.UP && (!solidBlock || /* This can improve glowstone wiring up to 2.5x but can change the update order */ IonConfig.optimisedGlowstone && block == Blocks.GLOWSTONE) || facingVertical == EnumDirection.DOWN && solidBlock && !worldIn.getType(offsetPos.shift(facingHorizontal)).getBlock().isOccluding()) {
